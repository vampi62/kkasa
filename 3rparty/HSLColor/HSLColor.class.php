...
class HSLColor
{
	...
	public function changeLightness( $amount )
	{
		...
		if ($this->_lightness > 1.0) {
			$this->_lightness = 1.0;
		} elseif ($this->_lightness < 0.0) {
			$this->_lightness = 0.0;
		}
	}
	...
	{
		$length = strlen( $html_string );

		if ( $length == 3 ) {
			$r = substr( $html_string, 0, 1 );
			$g = substr( $html_string, 1, 1 );
			$b = substr( $html_string, 2, 1 );

			$r = hexdec( $r . $r );
			$g = hexdec( $g . $g );
			$b = hexdec( $b . $b );
		} else {
			$r = substr( $html_string, 0, 2 );
			$g = substr( $html_string, 2, 2 );
			$b = substr( $html_string, 4, 2 );
